...
class DetectionLocalDatasourceImpl implements DetectionLocalDatasource {
  ...
  /// one isolate. It is always reset in `finally`, even on exceptions.
  ...
  Future<List<Map<String, dynamic>>> runInference(
    ...
    try {
      ...
      replyPort?.close();
      // Always reset _isolateBusy, whether inference succeeds, times out, or
      // throws, so the camera stream never stays frozen.
      _isolateBusy = false;
    }
  }
...
// Isolate globals
// These variables live inside the isolate's own memory space and are never
// shared with the main isolate, so using globals here is safe.
  ...
  jobPort.listen((msg) {
    if (msg is _IsolateInitMsg) {
      ...
      try {
        ...
        _cachedInterpreter = Interpreter.fromBuffer(
...
/// Allocates a reusable output buffer shared across frames.
/// Reuses the previous buffer when the shape is unchanged to reduce GC pressure.
...
/// Parses a YOLOv8 output tensor from a flat [Float32List], supporting both:
/// - `[numBoxes x numChannels]` (standard)
/// - `[numChannels x numBoxes]` (transposed)
///
...
List<Map<String, dynamic>> _parseFlat({
  ...
  // Pre-filter the top 100 boxes before NMS to cap `O(n^2)` work.
...
/// Applies sigmoid only when the exported model uses raw logits.
/// Standard YOLOv8 exports already contain normalized scores.
double _activateScore(double rawScore) {
  if (!AppConstants.yoloOutputLogits) return rawScore;
  return 1.0 / (1.0 + exp(-rawScore));
}

/// Greedy per-class NMS: sort by descending score, then drop boxes whose IoU
/// exceeds the threshold against an already selected box in the same class.
