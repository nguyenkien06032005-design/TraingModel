...
class BoxTracker {
  ...
  List<SmoothedBox> update(List<DetectionObject> detections, {DateTime? now}) {
    ...
    for (final det in detections) {
      ...
      for (final entry in _tracked.entries) {
        ...
        final iou = _iou(
          tracked.left, tracked.top, tracked.width, tracked.height,
  ...
  double _iou(
    double al, double at, double aw, double ah,
...
class BoundingBoxPainter extends CustomPainter {
  ...
  void _drawLabel(Canvas canvas, Size size, Rect rect, String label, Color color) {
  ...
  Color _colorForLabel(String label) {
    const palette = [
      Color(0xFF00E676), Color(0xFF00B0FF), Color(0xFFFF6D00),
      Color(0xFFD500F9), Color(0xFF00E5FF), Color(0xFFFF4081),
    ];
